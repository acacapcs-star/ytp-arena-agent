...
class Strategy(BaseStrategy):
    ...
    def _simulation_horizon(self, state=None):
        if state is not None and len(state.get("chests", [])) <= ENDGAME_CHEST_THRESHOLD:
            return SIMULATION_HORIZON + ENDGAME_HORIZON_BONUS
        if self.use_trap_right_opening:
            return 5.0
        if state is not None and len(state.get("obstacles", [])) >= 18:
            return 5.5
        return SIMULATION_HORIZON

    def _beam_width(self, state):
        return ENDGAME_BEAM_WIDTH if len(state.get("chests", [])) <= ENDGAME_CHEST_THRESHOLD else BEAM_WIDTH
    ...
    def _simulate_plan(self, state, first_chest, route, save_path=False):
        ...
        def remove_opponent_captures(beam, current_time):
            ...
            for key in beam["remaining"]:
                if key == beam["current_target_key"] and not beam["capture_times"]:
                    continue
        ...
        def endpoint_residual_value(beam):
            if (
                beam["current_target"] is None
                or len(beam["capture_times"]) >= MAX_ROLLOUT_CHESTS
            ):
            ...
            return ROLLOUT_RESIDUAL_VALUE_WEIGHT * max(
                0.0,
                1.0 - remaining_eta / max(horizon, EPS),
            )
            ...
            for key, capture_time in zip(
                ...
                if lead > -0.10:
                    penalty += (
                        ...
                        * min(1.0, (lead + 0.10) / max(horizon, EPS))
                    )
            return penalty
